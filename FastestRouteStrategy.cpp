...
std::vector<Place*> FastestRouteStrategy::getRoute(Place* start, Place* end) {
    if (!start || !end){
        return {};
    }
    ...
    while (!unvisited.empty()){
        ...
        for (Place* neighbour : current->getNeighbours()){
            ...
            double newDist = dist[current] + current->getTravelTimeTo(neighbour);
    ...
    //has same implementation to shortest route but uses getTravelTimeTo instead
}